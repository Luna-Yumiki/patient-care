...
public class PatientDirectory {
    ...
    public ArrayList<Patient> getAllInfectious() {
        ArrayList<Patient> temp = new ArrayList<Patient>();
        for (Patient p : patients) {
            if (p.isInfectious()) {
                temp.add(p);
        ...
        return temp; // has the list of encounters who are infectious
    }

    public ArrayList<Patient> getAllInfectiousAndConfirmedPositive() {
        ArrayList<Patient> temp = new ArrayList<Patient>();
        for (Patient p : patients) {
            if (p.isInfectious() && p.isConfirmedPositive()) {
                temp.add(p);
            }
        }
        return temp; // has the list of encounters who are infectious
    ...
    public ArrayList<Patient> getAllVitalAbnormal() {
        ArrayList<Patient> temp = new ArrayList<Patient>();
        for (Patient p : patients) {
            if (!p.isVitalNormal()) {
                temp.add(p);
